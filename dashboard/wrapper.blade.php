<script>
    ...
    (async function() {
        ...
        const isExtension = '{identifier}' === 'pterosort';
        const DRAGLOCK_KEY = 'dragLock';
        const STORAGE_KEY_YOURS = 'order';
        const STORAGE_KEY_OTHERS = 'order_others';
        const STORAGE_KEY_CATEGORIES_YOURS = 'categories';
        const STORAGE_KEY_CATEGORIES_OTHERS = 'categories_others';
        ...
        let dragLockEnabled = await loadDragLock();
        let categories = [];

        const csr = getFirstGroup(/value="(\w+)"/gm, '{{csrf_field()}}')
        function saveToDB(value) {
            ...
            xhr.open("POST", `${window.location.href}extensions/{identifier}`, true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.onload = function() {
                console.log('PteroSort: Data saved to DB.')
            };
            ...
            return new Promise((res, rej) => {
                ...
                xhr.onreadystatechange = function() {
                    if (xhr.readyState == XMLHttpRequest.DONE) {
                        console.log(`PteroSort: Data '${key}' loaded from DB.`)
                        res(xhr.responseText)
                    }
                }
                xhr.open('GET', `${window.location.href}extensions/{identifier}/${key}`, true);
                xhr.send(null);
            })
        }

        function saveJSONtoDB(key, json) {
            var obj = {};
            obj[key] = JSON.stringify(json);
            saveToDB(obj);
        }

        function saveDragLock(value) {
            if (isExtension) {
                var obj = {};
                obj[DRAGLOCK_KEY] = value ? 1 : 0;
                saveToDB(obj);
            } else {
                localStorage.setItem(DRAGLOCK_KEY, value);
            }
        }

        async function loadDragLock() {
            if (isExtension) {
                return +(await getFromDB(DRAGLOCK_KEY));
            } else {
                return localStorage.getItem(DRAGLOCK_KEY) === 'true';
            }
        }

        async function loadCategories() {
            if (isExtension) {
                categories = JSON.parse(await getFromDB(getCategoryStorageKey()) || '[]');
            } else {
        ...
        function saveCategories() {
            if (isExtension) {
                var obj = {};
                obj[getCategoryStorageKey()] = JSON.stringify(categories);
                saveToDB(obj);
            } else {
        ...
        async function loadOrderFromStorage() {
            if (isExtension) {
                return JSON.parse(await getFromDB(getStorageKey()) || '[]');
            } else {
        ...
        function saveOrderToStorage(order) {
            if (isExtension) {
                var obj = {};
                obj[getStorageKey()] = JSON.stringify(order);
                saveToDB(obj);
            } else {
            ...
            for (const child of container.children) {
                ...
                if (child.matches(serverSelector) && !child.classList.contains(categoryRowClass)) {
                    order.push({
                        ...
                        categoryId: child.dataset.categoryId || null
                    });
        ...
        async function exportSettings() {
            const settings = {
                ...
                categories_yours: JSON.parse(await getFromDB(STORAGE_KEY_CATEGORIES_YOURS) || '[]'),
                categories_others: JSON.parse(await getFromDB(STORAGE_KEY_CATEGORIES_OTHERS) || '[]'),
                order_yours: JSON.parse(await getFromDB(STORAGE_KEY_YOURS) || '[]'),
                order_others: JSON.parse(await getFromDB(STORAGE_KEY_OTHERS) || '[]'),

                dragLockEnabled: await getFromDB()
            };
        ...
        function importSettings(settings) {
            ...
            if (settings.categories_yours) saveJSONtoDB(STORAGE_KEY_CATEGORIES_YOURS, settings.categories_yours)
            if (settings.categories_others) saveJSONtoDB(STORAGE_KEY_CATEGORIES_OTHERS, settings.categories_others);
            if (settings.order_yours) saveJSONtoDB(STORAGE_KEY_YOURS, settings.order_yours);
            if (settings.order_others) saveJSONtoDB(STORAGE_KEY_OTHERS, settings.order_others);

            if (settings.dragLockEnabled !== undefined && settings.dragLockEnabled !== null) saveDragLock(settings.dragLockEnabled);
        }
        ...
        function openClearConfirmationOverlay() {
            ...
            document.getElementById('confirmDeleteAll').addEventListener('click', () => {
                saveJSONtoDB(STORAGE_KEY_CATEGORIES_YOURS, {})
                saveJSONtoDB(STORAGE_KEY_CATEGORIES_OTHERS, {});
                saveJSONtoDB(STORAGE_KEY_YOURS, {});
                saveJSONtoDB(STORAGE_KEY_OTHERS, {});
                saveDragLock(false);
        ...
        function observeViewSwitch() {
            ...
            if (toggleSwitch) {
                toggleSwitch.addEventListener('change', () => {
                    ...
                    setTimeout(() => {
                        waitForElement(serverSelector, async () => {
